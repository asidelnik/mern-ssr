...
const catsRouterV1 = express.Router();

// GET - most liked cats
catsRouterV1.get('/most-liked', async (_req: Request, res: Response) => {
  try {
    if (!collections.cats) {
...
catsRouterV1.get('/search', async (_req: Request, res: Response) => {
  try {
    if (!collections.cats) {
...
// GET - cat by id
catsRouterV1.get('/:id', async (_req: Request, res: Response) => {
  try {
    if (!collections.cats) {
      res.status(500).json({ message: 'Error fetching data' });
    } else {
...
catsRouterV1.put('/add-like/:id', async (_req: Request, res: Response) => {
  try {
    if (!collections.cats) {
      res.status(500).json({ message: 'Error fetching data' });
    } else {
...
catsRouterV1.get('*', (_req: Request, res: Response) => {
  res.status(404).json({ message: 'Cat resource not found' });
});

export default catsRouterV1;
